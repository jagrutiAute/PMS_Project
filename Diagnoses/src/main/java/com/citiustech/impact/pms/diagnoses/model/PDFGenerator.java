...
public class PDFGenerator {
	...
	public static ByteArrayInputStream medicationPDFReport(List<DiagnosisSave> medications) {
		...
		try {
			...
			Paragraph para = new Paragraph("Patient Diagnosis", font);
...
//					"Reference Standard", "Strength").forEach(headerTitle -> {
			Stream.of("diagnosis_code", "diagnosis_description", "patientID", "diagnosis_is_deprecated", "Phy_Id").forEach(headerTitle -> {
			...
			for (DiagnosisSave med : medications) {
				PdfPCell idCell = new PdfPCell(new Phrase(med.getDiagnosis_code()));
				idCell.setPaddingLeft(4);
				idCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				idCell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(idCell);

				PdfPCell actIngCell = new PdfPCell(new Phrase(med.getDiagnosis_description()));
				...
				PdfPCell formCell = new PdfPCell(new Phrase(med.getDiagnosis_is_deprecated()));
