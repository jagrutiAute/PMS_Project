package com.citiustech.impact.pms.util;
...
public class PDFGenerator {
	...
	public static ByteArrayInputStream medicationPDFReport(List<MedicationSave> medications) {
		...
		try {
			...
			Paragraph para = new Paragraph("Patient Medication", font);
			para.setAlignment(Element.ALIGN_CENTER);
			document.add(para);
			document.add(Chunk.NEWLINE);

			PdfPTable table = new PdfPTable(5);
			float[] columnWidths = new float[] { 10f, 20f, 10f, 10f, 10f };
...
//					"Reference Standard", "Strength").forEach(headerTitle -> {
			Stream.of("Appl_No", "Active_Ingredient", "Drug_Name", "Form", "Phy_Id").forEach(headerTitle -> {
			...
			for (MedicationSave med : medications) {
				PdfPCell idCell = new PdfPCell(new Phrase(med.getApplNo()));
				idCell.setPaddingLeft(4);
				idCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				idCell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(idCell);

				PdfPCell actIngCell = new PdfPCell(new Phrase(med.getActiveIngredient()));
				...
				PdfPCell drugNameCell = new PdfPCell(new Phrase(String.valueOf(med.getDrugName())));
				...
				PdfPCell formCell = new PdfPCell(new Phrase(med.getForm()));
